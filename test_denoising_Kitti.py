...
def main():
    ...
    model.load_state_dict(torch.load('ckpt/SGDFormer-Kitti.pth'))
    model.eval()
    lpfunc = lpips.LPIPS(net='vgg').cuda()

    # Load Image - Kitti Stereo 2015
    target_img = cv2.imread('examples/Kitti/001_target.png').astype('float') / 255.0
    guidance_img = cv2.imread('examples/Kitti/001_guidance.png').astype('float') / 255.0
    ...
    # Save Images
    cv2.imwrite('results/001_Kitti_res.png', np.uint8(denoised_img * 255))
    cv2.imwrite('results/001_Kitti_noisy.png', np.uint8(noisy_img * 255))
