...
def main():
    ...
    model.load_state_dict(torch.load('ckpt/SGDFormer-Flickr.pth'))
    model.eval()
    lpfunc = lpips.LPIPS(net='vgg').cuda()

    # Load Image - Flickr1024
    target_img = cv2.imread('examples/Flickr/001_target.png').astype('float') / 255.0
    guidance_img = cv2.imread('examples/Flickr/001_guidance.png').astype('float') / 255.0
    ...
    # Save Images
    cv2.imwrite('results/001_flickr_res.png', np.uint8(denoised_img * 255))
    cv2.imwrite('results/001_flickr_noisy.png', np.uint8(noisy_img * 255))
